...
async fn main() {
    ...
    let url = "sqlite:/Users/fabiankrusch/Desktop/test.db";
    ...
    sqlx::migrate!("./migrations").run(&*state_pool).await.expect("Error!!!");
...
async fn register_user(State(pool): State<Conn>, Json(user): Json<User>) -> impl IntoResponse {
     let result = sqlx::query("INSERT INTO user (name, surname, username, password, id) values ($1,$2,$3,$4,$5)")
        .bind(&user.name)
        .bind(&user.surname)
        .bind(&user.username)
        .bind(&user.password)
    ...
    if result.is_ok() {
        return (StatusCode::OK, Json( Message {message: "yeay".to_string()}))
    }
    return (StatusCode::BAD_REQUEST, Json( Message {message: "".to_string()}))
...
async fn login(State(pool): State<Conn>, Json(login): Json<LoginRequest>) -> Result<Json<LoginResponse>, StatusCode> {
    ...
    if username == "admin" && password == "admin" {
        ...
        let row = sqlx::query!("SELECT * FROM user WHERE username = $1 and password = $2", username, password)
